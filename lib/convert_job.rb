...
class ConvertJob
  ...
  class << self
    ...
    end # def perform(upload_id, filename, density = 150)
    ...
    def get_the_source_file
      ...
      # FIXME: here we need to get contentType and from gridfs_file
    ...
    def convert_file(filename, density = 150)
      ...
      if !system("#{convert_cmd} -background white -density #{density} \"#{filename}\" \"#{output_filename}\" 2>&1")
